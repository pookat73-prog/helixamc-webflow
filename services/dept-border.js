...
function SEAM(){Array.prototype.forEach.call(document.querySelectorAll('[data-hx-seam]'),function(n){n.remove()});if(window.innerWidth>767)return;var cards=Array.prototype.slice.call(document.querySelectorAll('[class*="dept-card_"]')).filter(function(el){var r=el.getBoundingClientRect();return el.offsetParent&&r.width>4&&r.height>4});cards.forEach(function(el){var r=el.getBoundingClientRect(),above=null,gap=1e9;cards.forEach(function(o){if(o===el)return;var ro=o.getBoundingClientRect();var ov=Math.min(r.right,ro.right)-Math.max(r.left,ro.left);if(ov<Math.min(r.width,ro.width)*0.4)return;/* 같은 열(수평으로 겹치는) 카드만 위아래로 취급 */var g=r.top-ro.bottom;/* o 가 el 바로 위면 g≈0 */if(g>=-4&&g<gap){gap=g;above=o}});if(!above)return;if(getComputedStyle(el).position==='static')el.style.position='relative';var bridge=gap>2?Math.min(gap,12):0;/* 카드 사이 간격이 있으면 그 위까지 살짝 덮어 두 카드를 잇는다 */var sh=document.createElement('div');sh.setAttribute('data-hx-seam','1');/* 이음새(위 카드 쪽)로 응축: 완전 어둠(피크)을 위쪽에서 두툼하게 유지 후 빠르게 사라져 겹침이 또렷 */sh.style.cssText='position:absolute;left:0;top:'+(-bridge)+'px;width:100%;height:'+(24+bridge)+'px;pointer-events:none;z-index:14;background:linear-gradient(to bottom,rgba(13,17,23,1) 0%,rgba(13,17,23,1) 24%,rgba(13,17,23,.66) 50%,rgba(13,17,23,.24) 74%,rgba(13,17,23,0) 100%)';el.appendChild(sh)})}
