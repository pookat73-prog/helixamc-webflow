...
function SEAM(){Array.prototype.forEach.call(document.querySelectorAll('[data-hx-seam],[data-hx-rdark]'),function(n){n.remove()});/* 이전에 그림자 위로 올려둔 글씨 z-index 원복 */Array.prototype.forEach.call(document.querySelectorAll('[data-hx-lift]'),function(n){n.style.zIndex='';n.style.position='';n.removeAttribute('data-hx-lift')});/* 세로모바일(portrait + 폭≤767)에서만 카드 그림자. 가로(landscape)면 방향만으로 무조건 제외 — 높이 조건(≤500)으로 새던 케이스 방지. */if(!(window.matchMedia('(orientation: portrait)').matches&&window.innerWidth<=767))return;var cards=Array.prototype.slice.call(document.querySelectorAll('[class*="dept-card_"]')).filter(function(el){var r=el.getBoundingClientRect();return el.offsetParent&&r.width>4&&r.height>4});cards.forEach(function(el){var r=el.getBoundingClientRect(),below=null,gap=1e9;cards.forEach(function(o){if(o===el)return;var ro=o.getBoundingClientRect();var ov=Math.min(r.right,ro.right)-Math.max(r.left,ro.left);if(ov<Math.min(r.width,ro.width)*0.4)return;/* 같은 열(수평으로 겹치는) 카드만 위아래로 취급 */var g=ro.top-r.bottom;/* o 가 el 바로 아래면 g≈0 */if(g>=-4&&g<gap){gap=g;below=o}});if(!below)return;/* 아래에 카드가 있는 카드(=내·외·영·안)만 → 맨 아래 치과는 아래 카드가 없어 제외 */if(getComputedStyle(el).position==='static')el.style.position='relative';var sh=document.createElement('div');sh.setAttribute('data-hx-seam','1');/* 각 카드의 바닥에 얹는다: bottom:0 고정으로 그 카드 아래쪽에서 그림자가 시작(아래로 번지지 않음). 완전 어둠 밴드를 0~45% 로 도톰하게 유지 후 위로 소멸. */sh.style.cssText='position:absolute;left:0;bottom:0;width:100%;height:18px;pointer-events:none;z-index:5;background:linear-gradient(to top,rgba(13,17,23,1) 0%,rgba(13,17,23,1) 45%,rgba(13,17,23,.5) 75%,rgba(13,17,23,0) 100%)';el.appendChild(sh)});/* 오른쪽 어둠 — 모든 카드(치과 포함)가 어둠 속에서 등장하는 느낌: 오른쪽 끝이 가장 어둡고 왼쪽으로 갈수록 투명. 그림자는 이미지 위·글씨 밑 층(z-index 4). 카드 글씨(제목·부제 등)를 그림자 위(z-index 6)로 올려 어둡게 묻히지 않게 함. */cards.forEach(function(el){if(/dept-card_dt/i.test(el.className||''))return;/* 치과(dt)는 오른쪽 어둠도 제외 — 치과 카드엔 그림자 전혀 없음 */if(getComputedStyle(el).position==='static')el.style.position='relative';/* 글씨 등 카드 콘텐츠를 그림자 위로: 이미지(dept_image*)와 우리 장식(data-hx*)은 제외해 그림자 밑에 남김 */Array.prototype.forEach.call(el.children,function(c){if(!c.attributes)return;for(var i=0;i<c.attributes.length;i++){if(c.attributes[i].name.indexOf('data-hx')===0)return}if(/dept_image/i.test(c.className||''))return;if(getComputedStyle(c).position==='static')c.style.position='relative';c.style.zIndex='6';c.setAttribute('data-hx-lift','1')});var rd=document.createElement('div');rd.setAttribute('data-hx-rdark','1');rd.style.cssText='position:absolute;top:0;right:0;height:100%;width:28%;pointer-events:none;z-index:4;background:linear-gradient(to right,rgba(13,17,23,0) 0%,rgba(13,17,23,.4) 62%,rgba(13,17,23,1) 100%)';el.appendChild(rd)})}
