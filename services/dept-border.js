/* ================================================================
   ...
   모바일(세로/가로) 가로선 통일 + 페이드 시작점:
   - 카드마다 폭이 달라 위/아래 가로선 길이가 제각각이던 것 → R() 이 모든 dept
     카드의 clientWidth 중 최소값(HL)을 재서, 모든 카드 가로선을 그 길이로 통일
     (넓은 카드는 가로선이 짧아져 좁은 카드와 동일 길이). 그라데이션을 userSpace
     로 tT~tT+HL 구간에 두고 pad 로 이후를 투명 처리해 실제 그은 path 는 전체폭
     이어도 보이는 길이는 HL 로 같아짐.
   - 가로선 페이드는 HFS(0.6) 지점부터 시작 → 더 멀리서 풀어짐(그 전까진 실선).
   - MOB(=PMOB||LSMOB) 에서만 HL 적용. 데스크탑/태블릿은 HL=null → 전체폭 유지.
