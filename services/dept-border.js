...
function SEAM(){Array.prototype.forEach.call(document.querySelectorAll('[data-hx-seam],[data-hx-rdark]'),function(n){n.remove()});/* 이전에 그림자 위로 올려둔 글씨 z-index 원복 */Array.prototype.forEach.call(document.querySelectorAll('[data-hx-lift]'),function(n){n.style.zIndex='';n.style.position='';n.removeAttribute('data-hx-lift')});var DBG=/[?&]debug-seam=1/.test(location.search);function _dbg(m){if(!DBG)return;var d=document.getElementById('hx-seam-dbg');if(!d){d=document.createElement('div');d.id='hx-seam-dbg';d.style.cssText='position:fixed;top:0;left:0;right:0;z-index:2147483647;background:rgba(0,0,0,.88);color:#0f0;font:12px/1.4 monospace;padding:6px 8px;white-space:pre-wrap;pointer-events:none';(document.body||document.documentElement).appendChild(d);}d.textContent='[SEAM] '+m;}var _w=window.innerWidth,_h=window.innerHeight;if(window.innerWidth>767){_dbg('skip width>767  w='+_w+' h='+_h);return;}/* 데스크탑/태블릿 제외 */var cards=Array.prototype.slice.call(document.querySelectorAll('[class*="dept-card_"]')).filter(function(el){var r=el.getBoundingClientRect();return el.offsetParent&&r.width>4&&r.height>4});/* 모바일(세로·가로 공통): 카드 글씨를 카드 겹침 그림자(SH z-index 15) 위로 올려 글씨 끄트머리가 안 어둡게. 이미지(dept_image*)·우리 장식(data-hx*)은 그대로 그림자 밑에 남김. */cards.forEach(function(el){if(getComputedStyle(el).position==='static')el.style.position='relative';Array.prototype.forEach.call(el.children,function(c){if(!c.attributes)return;for(var i=0;i<c.attributes.length;i++){if(c.attributes[i].name.indexOf('data-hx')===0)return}if(/dept_image/i.test(c.className||''))return;if(getComputedStyle(c).position==='static')c.style.position='relative';c.style.zIndex='16';c.setAttribute('data-hx-lift','1')})});/* 세로모바일에서만 바닥 겹침 + 오른쪽 어둠. 방향 판정은 화면 비율(innerHeight≥innerWidth) — 인앱 브라우저(예: 네이버앱) matchMedia orientation 오보로 세로에서 그림자가 사라지던 문제 방지. */if(window.innerHeight<window.innerWidth){_dbg('LANDSCAPE(가로)로 판정 → 그림자 제외  w='+_w+' h='+_h+' cards='+cards.length);return;}cards.forEach(function(el){var r=el.getBoundingClientRect(),below=null,gap=1e9;cards.forEach(function(o){if(o===el)return;var ro=o.getBoundingClientRect();var ov=Math.min(r.right,ro.right)-Math.max(r.left,ro.left);if(ov<Math.min(r.width,ro.width)*0.4)return;/* 같은 열(수평으로 겹치는) 카드만 위아래로 취급 */var g=ro.top-r.bottom;/* o 가 el 바로 아래면 g≈0 */if(g>=-4&&g<gap){gap=g;below=o}});if(!below)return;/* 아래에 카드가 있는 카드(=내·외·영·안)만 → 맨 아래 치과는 아래 카드가 없어 제외 */if(getComputedStyle(el).position==='static')el.style.position='relative';var sh=document.createElement('div');sh.setAttribute('data-hx-seam','1');/* 각 카드의 바닥에 얹는다: bottom:0 고정으로 그 카드 아래쪽에서 그림자가 시작(아래로 번지지 않음). 완전 어둠 밴드를 0~45% 로 도톰하게 유지 후 위로 소멸. */sh.style.cssText='position:absolute;left:0;bottom:0;width:100%;height:18px;pointer-events:none;z-index:5;background:linear-gradient(to top,rgba(13,17,23,1) 0%,rgba(13,17,23,1) 45%,rgba(13,17,23,.5) 75%,rgba(13,17,23,0) 100%)';el.appendChild(sh)});/* 오른쪽 어둠 — 카드(치과 제외)가 어둠 속에서 등장하는 느낌: 오른쪽 끝이 가장 어둡고 왼쪽으로 갈수록 투명. z-index 4(이미지 위·글씨(16) 밑). 글씨 올리기는 위에서 모바일 공통으로 이미 처리. */cards.forEach(function(el){if(/dept-card_dt/i.test(el.className||''))return;/* 치과(dt)는 오른쪽 어둠 제외 — 그림자 전혀 없음 */if(getComputedStyle(el).position==='static')el.style.position='relative';var rd=document.createElement('div');rd.setAttribute('data-hx-rdark','1');rd.style.cssText='position:absolute;top:0;right:0;height:100%;width:28%;pointer-events:none;z-index:4;background:linear-gradient(to right,rgba(13,17,23,0) 0%,rgba(13,17,23,.4) 62%,rgba(13,17,23,1) 100%)';el.appendChild(rd)});_dbg('PORTRAIT(세로) 적용  w='+_w+' h='+_h+' cards='+cards.length+' seam='+document.querySelectorAll('[data-hx-seam]').length+' rdark='+document.querySelectorAll('[data-hx-rdark]').length)}
