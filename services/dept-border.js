/* ================================================================
   ...
   태블릿(768~991) 한정 변경:
   - 내과(im)·외과(sg) 도 이 뷰에서만 밑변 없는 역ㄱ자(위+왼쪽) + 왼쪽변 하단
     페이드로 그린다. 페이드는 안과(oc)의 OCL(55%부터 서서히) 대신 OCLT
     (82%까지 진하게 유지 후 급강하, 92%에서 완전 투명) 사용 → 끝만 가파르게
     풀리고 바닥 닿기 전에 사라짐.
     데스크탑/모바일은 그대로 ㄷ자(FULL, 위+왼쪽+밑변) 유지. R() 이 resize 마다
     재실행되므로 뷰 경계를 넘나들면 자동 재그림.
   - 안과(oc)에 드리운 것과 같은 어두운 드리움 그림자(SH)를 내과=바닥('bo'),
     외과=바닥('bo')+오른쪽('ri')에 추가. SH 에 바닥 방향('bo') 지원 추가
     (기존 좌/우 동작은 그대로). 이 그림자도 태블릿에서만.
   ================================================================ */
(function(){var NS='http://www.w3.org/2000/svg',SKY='#7dd3fc',BLU='#0075d6',SW=3,K=0.3;function E(t,a){var e=document.createElementNS(NS,t);for(var k in a)e.setAttribute(k,a[k]);return e}function G(id,x1,y1,x2,y2,sts){var g=E('linearGradient',{id:id,gradientUnits:'userSpaceOnUse',x1:x1,y1:y1,x2:x2,y2:y2});sts.forEach(function(s){g.appendChild(E('stop',{offset:s[0],'stop-color':s[1],'stop-opacity':s[2]}))});return g}function P(d,gid,solid){var at={d:d,fill:'none','stroke-width':SW,'stroke-linecap':'round','stroke-linejoin':'round'};at.stroke=solid?BLU:('url(#'+gid+')');return E('path',at)}function B(el,a,tz,nb,lg,co){Array.from(el.children).forEach(function(c){if(c.getAttribute&&c.getAttribute('data-hx-u'))c.remove()});if(getComputedStyle(el).position==='static')el.style.position='relative';if(tz)el.style.zIndex='11';var cs=getComputedStyle(el),w=el.clientWidth,h=el.clientHeight,o=SW/2;if(!w||!h)return;var rT=parseFloat(cs.borderTopLeftRadius)||0,rB=parseFloat(cs.borderBottomLeftRadius)||0,tT=Math.max(rT,o),tB=Math.max(rB,o);var u='u'+Math.random().toString(36).slice(2,7);var s=E('svg',{viewBox:'0 0 '+w+' '+h,width:w,height:h,fill:'none'});s.style.cssText='position:absolute;inset:0;pointer-events:none;overflow:visible;z-index:20';var d=E('defs');d.appendChild(G(u+'t',tT,0,w,0,[[0,SKY,1],[K,BLU,1],[1,BLU,a]]));if(lg&&!co)d.appendChild(G(u+'l',0,0,0,h,lg));if(!nb)d.appendChild(G(u+'b',tB,h,w,h,[[0,BLU,1],[1,BLU,a]]));s.appendChild(d);s.appendChild(P('M '+tT+' '+o+' L '+w+' '+o,u+'t'));if(co){if(rT>o)s.appendChild(P('M '+tT+' '+o+' A '+(rT-o)+' '+(rT-o)+' 0 0 0 '+o+' '+rT,null,true))}else if(lg){var lp='M '+tT+' '+o;lp+=rT>o?' A '+(rT-o)+' '+(rT-o)+' 0 0 0 '+o+' '+rT:' L '+o+' '+o;lp+=' L '+o+' '+(rB>o?h-rB:h-o);lp+=rB>o?' A '+(rB-o)+' '+(rB-o)+' 0 0 0 '+tB+' '+(h-o):' L '+tB+' '+(h-o);s.appendChild(P(lp,u+'l'))}if(!nb)s.appendChild(P('M '+tB+' '+(h-o)+' L '+w+' '+(h-o),u+'b'));s.setAttribute('data-hx-u','1');el.appendChild(s)}function A(sel,a,tz,nb,lg,co){document.querySelectorAll(sel).forEach(function(el){B(el,a,tz,nb,lg,co)})}function SH(sel,dir,deep){var attr='data-hx-sh-'+(dir||'l');document.querySelectorAll(sel).forEach(function(el){var old=el.querySelector('['+attr+']');if(old)old.remove();if(getComputedStyle(el).position==='static')el.style.position='relative';var w=el.clientWidth,h=el.clientHeight,reach=w*0.6,reachV=h*0.6;var C='rgba(13,17,23,';var s0=deep?8:5,s1=deep?60:55;/* deep=까만(불투명) 밴드를 조금 더 넓게(s0)+페이드도 살짝 멀리(s1) */var sh=document.createElement('div');sh.setAttribute(attr,'1');var css='position:absolute;pointer-events:none;z-index:15;';if(dir==='bo'){/* 바닥 드리움: 바닥변이 가장 어둡고 위로 갈수록 투명 */var gradB='linear-gradient(to top, '+C+'1) 0%, '+C+'1) '+s0+'%, '+C+'0) '+s1+'%, '+C+'0) 100%)';css+='left:0;bottom:0;width:100%;height:'+reachV+'px;background:'+gradB+';'}else{var grad='linear-gradient(to left, '+C+'1) 0%, '+C+'1) '+s0+'%, '+C+'0) '+s1+'%, '+C+'0) 100%)';css+='top:0;width:'+reach+'px;height:100%;background:'+grad+';';css+=(dir==='ri')?'right:0':'left:-'+reach+'px'}sh.style.cssText=css;el.appendChild(sh)})}function R(){var FULL=[[0,SKY,1],[K,BLU,1],[1,BLU,1]];var FADE=[[0,SKY,1],[K,BLU,1],[0.4,BLU,1],[0.5,BLU,0.2]];var OCL=[[0,SKY,1],[K,BLU,1],[0.55,BLU,1],[1,BLU,0]];var OCLT=[[0,SKY,1],[K,BLU,1],[0.82,BLU,1],[0.92,BLU,0]];var TAB=window.innerWidth>=768&&window.innerWidth<=991;if(TAB){/* 태블릿(768~991)만: 내과·외과를 밑변 없는 역ㄱ자 + 왼쪽변 끄트머리만 가파르게 푸는 페이드(OCLT: 82%까지 진하게 유지 후 급강하) */A('.dept-card_im,.dept-card_sg',0,0,1,OCLT);A('.dept-card_di,.dept-card_dt',0,0,false,FULL);/* 안과처럼 어두운 드리움 그림자(deep=까만 밴드 조금 더 넓게): 내과=바닥, 외과=바닥+오른쪽 */SH('.dept-card_im','bo',1);SH('.dept-card_sg','bo',1);SH('.dept-card_sg','ri',1)}else{A('.dept-card_im,.dept-card_sg,.dept-card_di,.dept-card_dt',0,0,false,FULL)}A('.dept-card_oc',0,0,1,OCL);SH('.dept-card_sg');SH('.dept-card_di');SH('.div-block-263');SH('.div-block-263','ri')}if(document.readyState!=='loading')R();else document.addEventListener('DOMContentLoaded',R);var T;window.addEventListener('resize',function(){clearTimeout(T);T=setTimeout(R,150)});
