...
/* SEAM — 세로로 쌓인 카드 사이에 그림자를 얹어 '위 카드가 아래 카드를 덮은' 겹침 느낌.
   ...
   세로 인접이 없어 자동 제외). 카드 겹침 inset 그림자는 모바일(≤767)에서만, 글씨(분과 제목·분과별 부연)를 겹침 그림자 위로 올리는 lift 는 모든 뷰(데스크탑 포함)에서 동작. z-index: 글씨 16(그림자 위) / 이미지 1(그림자 밑) / SH 겹침·드리움 15 / 테두리 20. */
function SEAM(){Array.prototype.forEach.call(document.querySelectorAll('[data-hx-seam],[data-hx-rdark]'),function(n){n.remove()});Array.prototype.forEach.call(document.querySelectorAll('[data-hx-inset]'),function(n){n.style.removeProperty('box-shadow');n.removeAttribute('data-hx-inset')});/* 이전에 올려둔 글씨 z-index 원복 */Array.prototype.forEach.call(document.querySelectorAll('[data-hx-lift]'),function(n){n.style.zIndex='';n.style.position='';n.removeAttribute('data-hx-lift')});var cards=Array.prototype.slice.call(document.querySelectorAll('[class*="dept-card_"]')).filter(function(el){var r=el.getBoundingClientRect();return el.offsetParent&&r.width>4&&r.height>4});/* 모든 뷰(데스크탑·태블릿·모바일 공통): 제목·부연만 카드 겹침 그림자(SH z15) 위로 올린다. header 자체는 올리지 않아 아래쪽 파란 경계선이 그림자 밑에서 자연스럽게 어두워지게 하고, 이미지(dept_image*)는 그대로 z1에 둔다. */cards.forEach(function(el){if(getComputedStyle(el).position==='static')el.style.position='relative';Array.prototype.forEach.call(el.children,function(c){if(!c.attributes)return;for(var i=0;i<c.attributes.length;i++){if(c.attributes[i].name.indexOf('data-hx')===0)return}var cn=c.className||'';if(/dept-card-header/i.test(cn)){Array.prototype.forEach.call(c.children,function(n){if(getComputedStyle(n).position==='static')n.style.position='relative';n.style.zIndex='16';n.setAttribute('data-hx-lift','1')});return}if(getComputedStyle(c).position==='static')c.style.position='relative';c.style.zIndex=/dept_image/i.test(cn)?'1':'16';c.setAttribute('data-hx-lift','1')})});if(window.innerWidth>767)return;/* 아래(카드 겹침 inset 그림자)는 모바일(≤767) 전용 — 글씨 lift 는 위에서 이미 모든 뷰에 적용됨. */ /* 세로모바일에서만 카드 그림자. 가로면 제외(방향은 화면 비율로 판정 — 인앱 브라우저 matchMedia 오보 방지). */if(window.innerHeight<window.innerWidth)return;/* 오른쪽 어둠 + 바닥 겹침을 inset box-shadow 로. inset 그림자는 요소 배경(사진) 위·글씨(콘텐츠) 밑에 그려져 z-index 싸움이 없다. 사진이 카드/header/dept_image 중 어디 배경이든 커버하려 세 후보 모두 적용(같은 브랜드블랙이라 겹쳐도 가장자리만 강화). 치과(dt) 제외. */cards.forEach(function(el){var isDt=/dept-card_dt/i.test(el.className||'');/* 오른쪽 어둠은 치과 포함 전 카드. 바닥 겹침은 아래 카드가 있는 카드만(=치과 제외). 폭은 offset 축소로 좁게. */var bs='inset -42px 0 22px -16px rgba(13,17,23,1)'+(isDt?'':',inset 0 -26px 15px -10px rgba(13,17,23,1)');[el,el.querySelector('[class*="dept-card-header"]'),el.querySelector('[class*="dept_image"]')].forEach(function(t){if(!t)return;t.style.setProperty('box-shadow',bs,'important');t.setAttribute('data-hx-inset','1')})})}function R(){var FULL=[[0,SKY,1],[K,BLU,1],[1,BLU,1]];var FADE=[[0,SKY,1],[K,BLU,1],[0.4,BLU,1],[0.5,BLU,0.2]];var OCL=[[0,SKY,1],[K,BLU,1],[0.55,BLU,1],[1,BLU,0]];var TAB=window.innerWidth>=768&&window.innerWidth<=991;/* 가로모바일: landscape-mobile.js 와 동일 기준(가로 방향 + 높이 ≤500 + ≤767). 이 뷰에서만 안과를 치과와 동일한 ㄷ자(FULL, 밑변 있는 실선)로 그림 */var LSMOB=window.matchMedia('(orientation: landscape)').matches&&window.innerHeight<=500&&window.innerWidth<=767;/* 세로모바일: 세로 방향 + 폭 ≤767. 이 뷰에선 치과 제외 전부 역ㄱ자(OCL) */var PMOB=window.matchMedia('(orientation: portrait)').matches&&window.innerWidth<=767;/* 가로모바일: 영상의학과(di) 카드 높이를 내과(im)(없으면 외과 sg) 카드에 맞춰 복사. services.css 가 세로폰용으로 걸어둔 height:auto(텍스트 높이로 접힘) 를 인라인 !important 로 이겨 내외과와 동일 높이로. 그 외 뷰에선 인라인 해제해 services.css 규칙 복원. 높이 변경 후 아래 A() 가 이 높이로 테두리를 다시 그림. */(function(){/* 카드 클래스는 숨은 데스크탑 덱(dept-container-dt, 모바일 display:none → width 0)과 보이는 모바일 덱 양쪽에 있음. querySelector 는 숨은 덱을 먼저 잡으므로, 보이는(width>0) 인스턴스만 고른다. */function vis(sel){var r=null;document.querySelectorAll(sel).forEach(function(e){if(e.getBoundingClientRect().width>0)r=e});return r}var di=vis('.dept-card_di'),im=vis('.dept-card_im')||vis('.dept-card_sg');if(!di)return;if(LSMOB&&im){var rh=im.getBoundingClientRect().height;if(rh>0){di.style.setProperty('box-sizing','border-box','important');di.style.setProperty('height',rh+'px','important');di.style.setProperty('min-height',rh+'px','important');di.style.setProperty('align-self','start','important')}}else{di.style.removeProperty('box-sizing');di.style.removeProperty('height');di.style.removeProperty('min-height');di.style.removeProperty('align-self')}if(PMOB){/* 세로모바일(1열): 영상 칸만 justify-self=auto(내용폭) 라 좁게 나옴 → 다른 카드처럼 stretch + 전체폭 강제 */di.style.setProperty('justify-self','stretch','important');di.style.setProperty('width','100%','important');di.style.setProperty('max-width','none','important')}else{di.style.removeProperty('justify-self');di.style.removeProperty('width');di.style.removeProperty('max-width')}})();/* 모바일(세로/가로): 모든 카드의 가로선(위/아래)을 가장 좁은 카드 폭(HL)으로 통일. 카드마다 폭이 달라도 가로선 길이가 같아짐. 페이드는 HFS 지점부터 시작(더 멀리서 풀어짐). 데스크탑/태블릿은 HL=null 로 전체폭 유지. */var MOB=PMOB||LSMOB;var HL=MOB?1:null;if(TAB){/* 태블릿(768~991)만: 내과·외과를 안과처럼 밑변 없는 역ㄱ자 + 왼쪽변 하단 페이드(OCL) + 안과식 드리움 그림자(내과=바닥, 외과=바닥+오른쪽) */A('.dept-card_im,.dept-card_sg',0,0,1,OCL);A('.dept-card_di',0,0,false,FULL);A('.dept-card_dt',0,0,1,OCL);SH('.dept-card_im','bo');SH('.dept-card_sg','bo');SH('.dept-card_sg','ri')}else if(LSMOB){/* 가로모바일: 내·외·영은 역ㄱ자(밑변 없음 + 왼쪽 세로선 하단 페이드 OCL), 치과만 ㄷ자(FULL). 안과는 아래에서 FULL 로 그림. 가로선은 HL 로 통일 */A('.dept-card_im,.dept-card_sg,.dept-card_di',0,0,1,OCL,false,null,HL);A('.dept-card_dt',0,0,1,OCL,false,null,HL)}else if(PMOB){/* 세로모바일: 치과 제외 전부 역ㄱ자(OCL). 안과는 아래 else 에서 OCL 로 그려짐. 가로선은 HL 로 통일 */A('.dept-card_im,.dept-card_sg,.dept-card_di',0,0,1,OCL,false,null,HL);A('.dept-card_dt',0,0,1,OCL,false,null,HL)}else{A('.dept-card_im,.dept-card_sg,.dept-card_di',0,0,false,FULL);A('.dept-card_dt',0,0,1,OCL)}if(LSMOB){/* 가로모바일: 안과도 치과와 동일하게 ㄷ자(밑변 있는 실선). 가로선은 HL 로 통일 */A('.dept-card_oc',0,0,false,FULL,false,null,HL)}else{/* 그 외: 역ㄱ자(OCL). 세로모바일이면 HL 로 통일, 데스크탑/태블릿은 HL=null 로 전체폭 */A('.dept-card_oc',0,0,1,OCL,false,null,HL)}SH('.dept-card_sg');SH('.dept-card_di');SH('.div-block-263');SH('.div-block-263','ri');SEAM()}if(document.readyState!=='loading')R();else document.addEventListener('DOMContentLoaded',R);var T;window.addEventListener('resize',function(){clearTimeout(T);T=setTimeout(R,150)});window.addEventListener('orientationchange',function(){clearTimeout(T);T=setTimeout(R,220)});
