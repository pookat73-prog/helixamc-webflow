/* ================================================================
   ...
   - 화살표를 유니코드(\2192) → 폰트 독립 SVG 로 교체 (카드 폰트에 → 글리프가
     없어 "◀92" 두부로 깨지던 문제).
   - 원 테두리를 불투명 블루 + 살짝 어두운 원 배경으로 → 사진 위에서도 또렯히.
   ...
   - dept-border.js 가 로드 시 고정 크기로 그린 정적 장식이 카드 scale 을 안
     ...
     · 안과/치과가 든 오른쪽 열 컨테이너(div-block-263)의 테두리([data-hx-u])가
       안쪽 카드만 커질 때 떠 파란 선으로 어긋나던 문제
     → 그리드 호버 동안 이 정적 장식들을 부드럽게 숨김(각 카드 자체 테두리만
       남아 스케일과 함께 정확히 움직임).

   상세페이지 연결
   - 상세페이지를 만들면 아래 LINKS 값만 채우면 됨 (Webflow 손대질 필요 없음).
     예) im: '/naegwa'
   ================================================================ */
(function () {
  'use strict';
  ...
  var CSS =
    ...
    /* 화살표 버튼(원) — 다크 배경 + 블루 테두리로 항상 고정(호버해도 안 변함).
       z-index 16 으로 격침 그림자(SH, z-index 15) 위에 올림(카드 z-index 제거로
    ...
    /* 영상 카드 이미지 강제 표시 — Webflow 게시본 버그 우회.
       증상: 디자이너 캤버스엔 영상 이미지가 보이는데, 라이브 태블릿(768~991)에서만
       display:none 으로 사라짐. Webflow 게시 CSS 가 디자인과 어긋나며 이 셀만 숨김을 내보내며,
       재게시·요소 재생성으로도 안 지워짐(플랫폼 단 꿈김). → 사이트가 원래 주입하는 이 CSS 로
       !important 강제 표시해 그 숨김을 이긴다.
  ...
  function forceDiTablet() {
    ...
    if (window.innerWidth >= 768 && window.innerWidth <= 991) {
      ...
      /* 클립만(상한 없음) — 좋은 태블릿(미니)에선 넘침을 잘라내고, 큰 태블릿
         (에어)에선 이미지가 카드 높이만큼 늘어나 검은 띄가 안 생기게.
