...
>
> 진행 상태 (2026-09-01): 달별 정리·구조 내보내기 스크립트는 **붙여넣기 완료**.
> 요약 표 14개를 그리는 `scripts/sheet-summary-build.gs` 가 새로 들어왔다 —
> 붙여넣고 메뉴 [📊 요약 표(새 방식)] → [지금 새로 만들기] 를 눌러 새 탭을 만든 뒤,
> [기준값과 대조하기] 로 8월 숫자가 맞는지 확인하는 것이 다음 차례.
> **순서 주의**: 옛 수식(IMPORTRANGE 8벌)을 걷어낸 뒤에 로그 달별 나누기를 켤 것.
> 반대로 하면 요약 표에서 지난달치가 사라진 것처럼 보인다.
