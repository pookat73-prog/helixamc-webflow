...
> · ⚠️ **2026-09-02: 표 24개로 또 늘었다(표24 = 측정 이력). 다시 붙여넣을 것.**
>   ① 옛 백분율 서식이 남아 `4768` 이 `476800.0%` 로 보이던 것 수정
>   (`clear()` 가 숫자 서식을 못 걷는다 → `clearFormats()` + '자동' 서식 2중 처리)
>   ② **표24 신설** — 이벤트 이름별 첫 기록·마지막 기록·건수를 로그에서 직접 읽는다.
>   숫자가 0일 때 "측정이 안 붙은 건가 / 나중에 붙은 건가 / 아무도 안 누른 건가"를
>   **코드 이력 뒤지지 말고 이 표부터 볼 것** (2026-09-02 에 그러다 시간 버렸다)
