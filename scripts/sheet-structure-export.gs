/* ================================================================
   ...
   ── 쓰는 법 ──────────────────────────────────────────────────
   방법 A (이 파일 하나만 넣었을 때 — 가장 간단)
   1) 요약 스프레드시트에서 [확장 프로그램] → [Apps Script]
   2) 이 파일 내용을 붙여넣고 저장(Ctrl+S)
   3) 위쪽 함수 목록에서 exportSummaryStructure 를 고르고 ▶ 실행
   4) 처음 한 번은 권한 허용 창이 뜬다 (본인 계정이므로 허용)

   방법 B (메뉴로)
   - 상단 메뉴 [🧰 진단] → [요약 시트 구조 내보내기]
     (메뉴는 sheet-dwell-journey.gs 가 같은 프로젝트에 있어야 생긴다)

   어느 쪽이든 드라이브에 '헬릭스 시트구조 …' 라는 글 파일이 생긴다.
   Claude 는 그 이름으로 드라이브를 뒤져 찾아 읽을 수 있으므로,
   사용자는 '돌렸다' 고만 말해 주면 된다.
   ================================================================ */
...
function exportSummaryStructure() {
  ...
  /* 어디서 실행하든 결과를 알 수 있게 세 군데에 남긴다.
     - 메뉴로 눌렀으면 창으로
     - Apps Script 편집기의 ▶ 실행으로 돌렸으면 창이 안 뜰 수 있으니 토스트로
     - 둘 다 막혀도 실행 기록(Logger)에는 남는다
     셋 다 실패해도 파일은 이미 만들어져 있으므로, 여기서 멈추지 않게 감싼다. */
  try { Logger.log(msg); } catch (e0) {}
  try {
    SpreadsheetApp.getUi().alert('시트 구조 내보내기', msg, SpreadsheetApp.getUi().ButtonSet.OK);
  } catch (e1) {
    try { SpreadsheetApp.getActive().toast(name + ' 를 만들었습니다.', '시트 구조', 15); } catch (e2) {}
  }
  return name;
}
