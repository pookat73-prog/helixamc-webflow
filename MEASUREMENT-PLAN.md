...
- [x] (다, 채택) `IMPORTRANGE` 자체를 걷어내고 "체류·동선" 탭처럼 Apps Script가 로그를 직접 읽어 쓰는 방식으로 전환 — **2026-09-01 구현**. `scripts/sheet-summary-build.gs` 가 표 14개를 새 탭 [요약(새 방식)]에 그린다. 기존 요약 탭은 안 건드림(숫자 대조 후 갈아타기). 옛 수식 162자리를 그대로 옮긴 것이라 집계 규칙은 일부러 안 고쳤다(표5·표8 전화 세는 법 차이, 표1 합계 89 차이 — 둘 다 코드 주석에 이유 있음). `verifySummaryAgainstBaseline()` 이 2026년 8월 기준값 32개 + 항등식 6개를 자동 대조한다.
  - 남은 일 🖱: 사용자가 Apps Script 에 붙여넣기 → [지금 새로 만들기] → [기준값과 대조하기] → 다 맞으면 옛 수식·IMPORTRANGE 8벌 제거 → **그 다음에** 로그 달별 나누기 켜기 (순서 바뀌면 요약에서 지난달치가 사라진 것처럼 보임)
